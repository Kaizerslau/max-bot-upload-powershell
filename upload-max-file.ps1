...
Write-Host "Upload type: $resolvedUploadType"
Write-Host "MIME type: $mimeType"
if ($requestedUploadType -eq "auto") {
    Write-Host "Type source: auto by file extension"
}
else {
    Write-Host "Type source: explicit"
}
...
$curlArgs = @(
    "-sS",
    "-X", "POST",
    "-F", "data=@$localFilePath;type=$mimeType",
    $uploadUrl
)
...
$uploadResult = Read-UploadResult -RawResponse $uploadRawResponse
...
if ($resolvedUploadType -eq "audio" -or $resolvedUploadType -eq "video") {
    ...
    if ([string]::IsNullOrWhiteSpace($mediaToken)) {
        Fail "Upload succeeded, but no media token was returned for $resolvedUploadType."
    }
    ...
    if ($payload -eq $null) {
        Fail "Upload succeeded, but no JSON payload or token was returned for $resolvedUploadType."
    }
}
...
Write-Host (ConvertTo-PrettyJson -Object $payload)
